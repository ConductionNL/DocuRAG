...
def build_or_update_faiss(
    emb: np.ndarray,
    items: List[Dict],
    index_path="woo.faiss",
    map_path="woo_idmap.json",
    meta_path="woo_meta.json",
):
    ...
    with open(meta_path, "w") as f:
        json.dump(
            {
                it["vector_id"]: {k: v for k, v in it.items() if k != "text"}
                for it in items
            },
            f,
        )
...
def search(
    ...
    for rank, (score, pos) in enumerate(zip(D[0], I[0]), 1):
        vid = inv[pos]
        results.append(
            {"rank": rank, "score": float(score), "vector_id": vid, **meta[vid]}
        )
    return results
