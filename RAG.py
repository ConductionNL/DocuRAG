import json
from embeddings import index_docs, search
...
def retrieve_docs(corpus_path: str,query: str, top_k: int = 5) -> list[dict]:
    """Retrieve top-k documents relevant to the query from the corpus.
...
def main(doc_path: str, user_input: str) -> str:
    """Run retrieval-augmented generation for the given inputs.

    @param doc_path: Path to the JSON corpus file.
    @param user_input: Natural-language query from the user.
    @return: The parsed LLM response string.
    """
    retrieved_docs = retrieve_docs(doc_path, user_input, top_k=5)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python RAG.py <doc_path> <user_input>")
        sys.exit(1)
    doc_path = sys.argv[1]
    user_input = " ".join(sys.argv[2:])
    output = main(doc_path, user_input)
    print(output)
